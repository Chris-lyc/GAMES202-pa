        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    //球谐函数是一个对整个球面的函数，即对整个环境光进行投影，利用黎曼积分拆解成每个像素投影后加起来
                    ...
                    for(int l=0;l<=SHOrder;l++)
                    {
                        for(int m=-l;m<=l;m++)
                        {
                            ...
                            SHCoeffiecents[k]+=Le*sh::EvalSH(l,m,dir.cast<double>().normalized())*dw;
                        }
                    }
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed)
                {
                    ...
                    // 返回传输项函数值（unshadowed时实际就是cos），后面传入ProjectFunction进行投影算出系数
                    double H= wi.dot(n);
                    if(H>0)return H;
                    return 0;
                }
                else
                {
                    ...
                    //shadowed要考虑可见项 可见项要么是1要么是0
                    double H=wi.dot(n);
                    auto ray=Ray3f(v,wi);

                    if(H>0 && !scene->rayIntersect(ray))return H;
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            for(int i=0;i<mesh->getVertexCount();i++)
            {
                ...
                std::unique_ptr<std::vector<double>> shCoeff;
                std::unique_ptr<std::vector<double>> shCoeff1;
                ...
                auto shFunc=[&](double phi,double theta)->double{
                    ...
                    if(H>0.0)
                    {
                        ...
                        {
                            for(int i=0;i<inter.mesh->getVertexCount();i++)
                            {
                                const Point3f &v1 = inter.mesh->getVertexPositions().col(i);
                                const Point3f &n1 = inter.mesh->getVertexNormals().col(i);

                                auto shFunc1 = [&](double phi1, double theta1)->double{
                                    Eigen::Array3d d1=sh::ToVector(phi1,theta1);
                                    const auto wi1 = Vector3f(d1.x(), d1.y(), d1.z());
                                    double H1=wi1.dot(n1);

                                    if(H1>0.0)
                                        return H1;
                                    return 0;
                                };
                                shCoeff1 = sh::ProjectFunction(SHOrder, shFunc1, m_SampleCount);
                            }
                        }
                        return H;
                    }
                ...
                shCoeff=sh::ProjectFunction(SHOrder, shFunc, m_SampleCount);
                ...
                else
                {
                    for(int j=0;j<shCoeff->size();j++)
                    {
                        m_TransportSHCoeffs.col(i).coeffRef(j) = (*shCoeff)[j]+(*shCoeff1)[j];
                    }
                }
            }
        }
