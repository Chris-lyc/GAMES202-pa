//单个模型的绘制入口，注意 shadow 和 object 是分离成两个不同的 MeshRender 进行渲染的。
//draw() 方法中最终调用了 gl.drawElements() 开始了 WebGL 的正式绘制流程。
class MeshRender {
	...
	//在构造函数中，创建了 js 与 WebGL 间的数据绑定，即将 LoadOBJ 中加载的模型信息复制到 WebGL 的 Buffer 中。
	//在构造函数的最后，创建了 Shader 实例，完成 gl 的 Compile 和 Link。
